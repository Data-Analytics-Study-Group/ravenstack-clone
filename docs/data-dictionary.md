...
4. [support_tickets](#4-support_tickets)
5. [events](#5-events)
...
| `account_id` | ID (FK) | Foreign key referencing `accounts.account_id`. |
...
| `subscription_id` | ID (FK) | Foreign key referencing `subscriptions.subscription_id`. |
